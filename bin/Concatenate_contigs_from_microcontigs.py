...
def align_cmd(ref_file,microcontig_fasta,microcontig_sam,xin):
    try:
        ...
        use_cmd = code_path + "minimap2/" + "minimap2 -a " + ref_file + " " + microcontig_fasta + " > " + microcontig_sam 
...
def align_microcontigs(chr_start,chr_end,ref_file,out_dir):
    ...
    for chr_num in range(chr_start,chr_end + 1):
        microcontig_fasta = out_dir + "Aquila_cutPBHC_minicontig_chr" + str(chr_num) + ".fasta"
        microcontig_sam = out_dir + "Aquila_cutPBHC_minicontig_chr" + str(chr_num) + ".sam"
...
def Concatenate_wgs(chr_start,chr_end,out_dir):
    for chr_num in range(chr_start,chr_end + 1):
        one_contig = out_dir + "Aquila_Contig_chr" + str(chr_num) + ".fasta"
        cat_cmd = "cat " + one_contig  + " >> " + out_dir + "Aquila_contig.fasta"
